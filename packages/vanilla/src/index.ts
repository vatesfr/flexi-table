import {
  processData, groupData, computeStringValues, paginateData, calcTotalPages,
  toggleSort as coreToggleSort, toggleFilter as coreToggleFilter,
  toggleGroupBy, toggleCollapse, getSortIcon, getSortIndex,
  countActiveFilters, DEFAULT_LABELS,
  type SortEntry, type RangeFilter, type DataTableLabels,
...
export function createFlexiTable<TRow extends object>(
  ...
  function render(): void {
    ...
    const focusKey = focused && container.contains(focused) ? focused.dataset.focusKey ?? null : null
    ...
    // Columns
    html += buildDd(openDropdown === 'cols',
    ...
    // Sort
    html += buildDd(openDropdown === 'sort',
    ...
    if (stringFilterCols.length > 0 || numericFilterCols.length > 0) {
      html += buildDd(openDropdown === 'filter',
    ...
    if (groupableCols.length > 0) {
      html += buildDd(openDropdown === 'group',
    ...
    for (const { key: gkey, rows } of _groupedData) {
      if (gkey !== null) {
        ...
        for (let gi = 0; gi < groupBy.length; gi++) {
          ...
          html += gCol ? cellStr(rows[0], gCol) : esc(String((rows[0] as Record<string, unknown>)[gColKey] ?? ''))
        }
    ...
    if (selectable) {
      ...
      for (const { key: gkey, rows } of _groupedData) {
        ...
        if (gSomeSel) {
          for (const cb of container.querySelectorAll<HTMLInputElement>('[data-action="toggle-group-select"]')) {
            if (cb.dataset.gkey === gkey) { cb.indeterminate = true; break }
          }
        }
      }
    }
  ...
  function handleClick(e: MouseEvent): void {
    ...
    switch (action) {
      ...
      case 'page-prev': page = Math.max(1, _clampedPage - 1); break
      case 'page-next': page = Math.min(_numPages, _clampedPage + 1); break
      case 'page-last': page = _numPages; break
      default: return
    }
  ...
  function handleInput(e: Event): void {
    ...
    rangeFilters = {
      ...rangeFilters,
      [key]: { min: rangeFilters[key]?.min ?? '', max: rangeFilters[key]?.max ?? '', [field]: target.value },
    }
    page = 1
    render()
  }
  ...
  return {
    setData(newData: TRow[]): void { data = newData; render() },
    setColumns(newCols: ColumnDef<TRow>[]): void { columns = newCols; render() },
    destroy(): void {
